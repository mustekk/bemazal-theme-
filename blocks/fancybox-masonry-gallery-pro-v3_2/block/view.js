...
(function(){
  ...
  function initOne(root){
    ...
    function ensureFancyboxThenOpen(idx){
      function open(){ window.Fancybox.show(items, { Thumbs:{type:'classic'}, Toolbar:{display:['close','thumbs']}, Carousel:{ rtl: isRTL(root) }, startIndex: idx || 0 }); }
      if (!window.Fancybox) {
        Promise.all([ loadStyle(CDN.fancyboxCSS), loadScript(CDN.fancyboxJS) ]).then(open).catch(function(){});
      } else { open(); }
    }
